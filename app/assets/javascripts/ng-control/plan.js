    ...
    function($scope, $http, $attrs, $cookies, $sce, $window, $mdDialog) {
      ...
      $scope.select = function(object) {
        ...
        $scope.current_draggable =
          object &&
          (object.record_type === "Operation" ||
            object.record_type === "Module" ||
            object.record_type === "ModuleIO" ||
            object.record_type === "TextBox" ||
            object.record_type === "TextBoxAnchor")
            ? object
            : null;

        $scope.current_op =
          object && object.record_type === "Operation" ? object : null;

        $scope.current_io =
          object &&
          (object.record_type === "FieldValue" ||
            object.record_type === "ModuleIO")
            ? object
            : null;

        $scope.current_fv =
          object && object.record_type === "FieldValue" ? object : null;

        $scope.current_wire =
          object &&
          (object.record_type === "Wire" || object.record_type === "ModuleWire")
            ? object
            : null;
      };
      ...
      $scope.add_operation = function(operation_type) {
        ...
        if ($scope.plan.name === "Untitled Plan") {
          ...
          $scope.state.messages.push(
            "Changed name of untitled plan to " + op.operation_type.name
          );
        }
      };
      ...
      $scope.add_predecessor = function(io, obj, pred) {
        if (
          obj.record_type === "Operation" &&
          io.record_type === "FieldValue"
        ) {
          $scope.plan.add_wire_from(io, obj, pred).then(plan => {
          ...
          obj.record_type === "Module" &&
          io.record_type === "ModuleIO"
        ) {
          var module = obj,
            fv = io.destinations[0].io,
            op = io.destinations[0].op;

          $scope.plan.add_wire_from(fv, op, pred).then(plan => {
            let newop = plan.operations[plan.operations.length - 1];
            $scope.select(newop);
      ...
      $scope.add_successor = function(io, obj, suc) {
        if (
          ...
          let newop = $scope.plan.add_wire_to(io, obj, suc).then(plan => {
            let newop = plan.operations[plan.operations.length - 1];
            $scope.select(newop);
          ...
          $scope.plan.add_wire_to(fv, op, suc).then(plan => {
            let newop = plan.operations[plan.operations.length - 1];
            $scope.select(newop);
            if (newop.num_outputs > 0) {
              $scope.set_current_io(newop.outputs[0]);
      ...
      $scope.save = function(plan) {
        plan
          .save($scope.current_user)
          .then(saved_plan => {
            $scope.plan = saved_plan;
            $scope.select_uba_by_budget_id(
              $scope.current_user,
              $scope.plan.budget_id
            );
            $scope.select(null);
      ...
      $scope.revert_template = function(plan, openafter = true) {
        let confirm = $mdDialog
          ...
          .textContent(
            "Do you really want to revert this template to a normal plan?"
          )
          .ariaLabel("Revert")
          .ok("Yes")
          .cancel("No");
      ...
      $scope.change_status = function(operation, new_status) {
        let confirm = $mdDialog
          ...
          .title("Change status?")
          .textContent(
            "Really change status of operation " +
              operation.id +
              " to " +
              new_status +
              "?\n" +
        ...
        $mdDialog
          ...
          .then(() => {
            ...
            operation.set_status(new_status).then(() => {
              ...
              $scope.multiselect = {};
              $scope.state.messages.push(
                `Changed state of operation ${operation.id} to ${new_status}.` +
      ...
      $scope.delete_plan = function(p) {
        let confirm = $mdDialog
          ...
          .title("Delete Plan?")
          .textContent(
            'Do you really want to delete the plan "' + p.name + '"?'
          )
          .ariaLabel("Delete")
          .ok("Yes")
          .cancel("No");
        ...
        AQ.Plan.load(plan.id)
          .then(p => {
            ...
            $scope.select_uba_by_budget_id(
              $scope.current_user,
              $scope.plan.budget_id
            ); // in case this plan is already running
            $scope.wait = false;
            $scope.$apply();
          })
          .catch(error => {
            $scope.report_error(
              "Could not read plan '" + plan.name + "'.",
              error.message,
              error.stack
            );
            console.log(error);
      ...
      $scope.launch = function() {
        ...
        $scope.plan.uba = null;
        aq.each(
          $scope.current_user.user_budget_associations,
          uba => (uba.selected = false)
        );
      ...
      $scope.select_item = function(fv, item) {
        ...
        if ($scope.current_op.status != "planning") {
          $scope.state.messages.push(
            `Changed item for an operation in state '${$scope.current_op.status}'. Save this plan to effect your change.`
          );
        }
      };
      ...
      $scope.set_allowable_field_type = function(aft) {
        if ($scope.current_op) {
          ...
          $scope.current_io.origin.op.set_aft(
            $scope.current_fv.field_type,
            aft
          );
        } else {
          $scope.current_io.destinations[0].op.set_aft(
            $scope.current_fv.field_type,
            aft
          );
        }
      ...
      $scope.allowable_field_type_checked = function(aft) {
        if ($scope.current_op) {
          if (
            !$scope.current_op.form[$scope.current_fv.role][
              $scope.current_fv.name
            ]
          ) {
          ...
          return (
            $scope.current_io.origin.op.form[$scope.current_fv.role][
              $scope.current_fv.name
            ].aft_id == aft.id
          );
        } else if (
          $scope.current_io.destinations[0].op.form[$scope.current_fv.role][
            $scope.current_fv.name
          ]
        ) {
          return (
      ...
      $scope.choose_category = function(category) {
        $scope.state.category_index = $scope.operation_types.categories.indexOf(
          category
        );
      };
